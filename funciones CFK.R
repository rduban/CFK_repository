...
my_scores_rendim<-function(datos,grupo){
  ...
  if(grupo=="inicial1"){
    ...
    x$pc8<-ifelse(is.na(datos$pc16)==TRUE,0,ifelse(datos$pc16=="La segunda expresión es verdadera si “(a O b)” es verdadero",1,0))
    
    x$pc9<-ifelse(is.na(datos$pc17)==TRUE,0,ifelse(datos$pc17=="Imagen 3",1,0))
    
    x$pc10<-ifelse(is.na(datos$pc18)==TRUE,0,ifelse(datos$pc18=="Está equivocada, el resultado es 120.",1,0))
  }
  
  if(grupo=="avanzado1"){
    
    x$pc1<- ifelse(is.na(datos$pc1)==TRUE,0,ifelse(datos$pc1=="alfa, delta, gama, beta, épsilon",1,0))
    
    x$pc2<- ifelse(is.na(datos$pc2)==TRUE,0,ifelse(datos$pc2=="Cree que, si la condición se cumple, todo lo que sigue se va a ejecutar",1,0))
    
    x$pc3<- ifelse(is.na(datos$pc3)==TRUE,0,ifelse(datos$pc3=="Imagen 3",1,0))
    
    x$pc4<- ifelse(is.na(datos$pc4)==TRUE,0,ifelse(datos$pc4=="Cada uno tiene un número asignado y gana solamente si presiona su botón cuando sale este número",1,0))
    ...
    x$pc7<- ifelse(is.na(datos$pc7)==TRUE,0,ifelse(datos$pc7=="Esta función no imprime nunca nada" | datos$pc7=="Se generará un error por intentar imprimir luego de retornar max",1,0))
